...
for (const token of tokens) {
	...
	client.on("message", async (message) => {
			...
			try {
				...
				await axios
					...
					.then((res) => {
						...
						const embed = new Discord.MessageEmbed()
							.setAuthor("Appointments Are Required")
		...
		if (command == "p") {
			...
			try {
				await CoinGeckoClient.coins.all().then((res) => {
					for (coindata in res.data) {
						...
						if (input === data.symbol) {
							...
							const embed = new Discord.MessageEmbed()
								...
								.setFooter(
									message.guild.name +
										" | " +
										"Symbol Price Tracker - Powered by CoinGecko",
									message.guild.iconURL()
								);
					...
					// let data = res.data[0].market_data.current_price.usd;
					// fs.writeFile("coingecko.json", JSON.stringify(data), (err) => {
					//   //         //   // In case of a error throw err.
					//   if (err) throw err;
					// });
				});
		...
		if (command == "indexnft") {
			...
			try {
				await axios({
					method: "POST",
					url: "https://traitsurfer.app/api/blockchainscrape",
					...
					.then((res) => {
						if (res.data.success) {
							const embed = new Discord.MessageEmbed()
								.setColor("#f09719")
								.setDescription(
									`${input} contract has been fully indexed. Check it out at https://traitsurfer.app`
								)
		...
		if (command == "reindexnft") {
			const input = args[0];

			try {
				await axios({
					method: "POST",
					url: "https://traitsurfer.app/api/blockchainrescrape",
					headers: { "Content-Type": "application/json" },
					data: { contractAddress: input },
				})
					.then((res) => {
						if (res.data.success) {
							const embed = new Discord.MessageEmbed()
								.setColor("#f09719")
								.setDescription(
									`${input} contract has been fully reindexed. Check it out at https://traitsurfer.app`
								)
								.setFooter(message.guild.name, message.guild.iconURL());
							message.channel.send(embed);
						} else {
		...
		if (command == "removenft") {
			const input = args[0];
			try {
				await axios({
					method: "POST",
					url: "https://traitsurfer.app/api/deleteOne",
					headers: { "Content-Type": "application/json" },
					data: { contractAddress: input },
				})
					.then((res) => {
						if (res.data.success) {
							const embed = new Discord.MessageEmbed()
								.setColor("#f09719")
								.setDescription(
									`${input} contract has been fully removed. Check it out at https://traitsurfer.app`
								)
								.setFooter(message.guild.name, message.guild.iconURL());
							message.channel.send(embed);
						} else {
							const embed = new Discord.MessageEmbed()
								.setColor("#f09719")
								.setDescription(
									`${input} contract removing error, please make a ticket.`
								)
